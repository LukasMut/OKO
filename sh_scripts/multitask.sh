...
echo "Started odd-one-out multi-task learning $SGE_TASK_ID for $network at $(date)"

python main.py --out_path $out_path --data_path $data_path --network $network --dataset $dataset --task $task --distribution $dist --samples ${samples[@]} --optim $optim --sampling $sampling --min_samples $min_samples --n_classes $n_classes --batch_sizes ${batch_sizes[@]} --epochs ${epochs[@]} --etas ${etas[@]} --burnin $burnin --patience $patience --steps $steps --seeds ${seeds[@]} >> ${logdir}/${task}_${dist}.out

printf "Finished odd-one-out multi-task learning $SGE_TASK_ID for $network at $(date)\n"
