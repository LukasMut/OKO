...
def parseargs():
    ...
    aa("--data_path", type=str, help="path/to/original/dataset")
    aa(
        "--dataset",
        type=str,
        choices=["mnist", "fashionmnist", "cifar10", "cifar100", "imagenet"],
    )
    aa("--network", type=str, default="ResNet18", choices=utils.MODELS)
...
if __name__ == "__main__":
    ...
    model = train.get_model(model_config=model_config, data_config=data_config)
    ...
    if args.dataset == "cifar10":
        dataset = np.load(os.path.join(args.data_path, "test.npz"))
        images = dataset["data"]
        labels = dataset["labels"]
    else:
        dataset = torch.load(os.path.join(args.data_path, "test.pt"))
