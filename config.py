...
def get_configs(args, **kwargs):
    ...
    # dataset imbalance is a function of the shape param alpha
    data_config.alpha = kwargs.pop('shape')
    ...
    data_config.n_samples = kwargs.pop('n_samples')
    data_config.batch_size = kwargs.pop('batch_size')
    
    model_config = config_dict.ConfigDict()
    model_config.type = re.compile(
        r'[a-zA-Z]+').search(args.network).group()
    
    try:
        model_config.depth = re.compile(
            r'\d+').search(args.network).group()
